...
**Semantic model — measures work is COMPLETE, plus a fact_games schema change.** `workspace/the_offload_semantic_model.SemanticModel` (Direct Lake, TMDL, 8 tables: `fact_games`, `fact_standings`, `dim_teams`, `dim_competitions`, `dim_competition_seasons`, `dim_game_dates`, `dim_snapshot_dates`, `bridge_game_team`). Relationships are correctly wired (including `fact_standings.team_key -> dim_teams.team_key` and the `bridge_game_team` many-to-many for `fact_games` ↔ `dim_teams`).
...
**Report design — next step, not started in files yet.** A full `Design Brief:` YAML contract for 2 pages ("Standings & Form", "Fixtures & Results") was produced collaboratively but only ever existed in chat history. On 2026-08-28 it was re-issued as a standalone handoff brief for a new chat session (given directly to the user, not saved to a repo file) — ask the user for that handoff text if resuming this thread and it isn't in the current conversation. No PBIP/Report files exist yet; `powerbi-report-authoring` hasn't been invoked.
